...
IF NOT EXISTS (
    SELECT * FROM [__EFMigrationsHistory]
    WHERE [MigrationId] = N'20260319035849_AddSeguimientoTriage'
)
BEGIN
    CREATE TABLE [Seguimientos] (
        ...
        [DiagnosticoConfirmado] nvarchar(200) NULL,
        [NivelTriageCorrecto] bit NOT NULL,
        [NivelTriageReal] int NULL,
        [Observaciones] nvarchar(500) NULL,
        [MedicamentosIndicados] nvarchar(500) NULL,
        [RegistradoPor] nvarchar(max) NOT NULL,
        [FechaRegistro] datetime2 NOT NULL,
        CONSTRAINT [PK_Seguimientos] PRIMARY KEY ([Id]),
        CONSTRAINT [FK_Seguimientos_TriageRegistros_TriageId] FOREIGN KEY ([TriageId]) REFERENCES [TriageRegistros] ([Id]) ON DELETE CASCADE
    );
END;
...
BEGIN
    CREATE UNIQUE INDEX [IX_Seguimientos_TriageId] ON [Seguimientos] ([TriageId]);
END;
...
BEGIN
    INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
    VALUES (N'20260319035849_AddSeguimientoTriage', N'10.0.5');
END;
