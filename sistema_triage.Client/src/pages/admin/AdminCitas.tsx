...
function ModalCrearSlots({ onClose, onCreado }: { onClose: () => void; onCreado: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!form.fechaHoraInicio) { setError('Selecciona fecha y hora de inicio'); return }
    setLoading(true)
    setError('')
    try {
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
      <div className="bg-gray-900 border border-gray-800 rounded-2xl w-full max-w-md">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            <label className="block text-xs font-medium text-gray-400 mb-1">Fecha y hora de inicio *</label>
          ...
          <div>
            <label className="block text-xs font-medium text-gray-400 mb-1">Duración por slot (minutos)</label>
            <select value={form.duracionMinutos}
          ...
          <div>
            <label className="block text-xs font-medium text-gray-400 mb-1">Cantidad de slots consecutivos</label>
            <input type="number" min={1} max={20} value={form.cantidadSlots}
              onChange={e => setForm(p => ({ ...p, cantidadSlots: parseInt(e.target.value) }))}
              className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-blue-500" />
            <p className="text-xs text-gray-500 mt-1">
              Se crearán {form.cantidadSlots} slot{form.cantidadSlots > 1 ? 's' : ''} de {form.duracionMinutos} min cada uno
            </p>
          </div>
...
function ModalGestionarCita({ cita, onClose, onGestionada }: {
  cita: Cita
  onClose: () => void
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-2xl w-full max-w-md">
        ...
        <div className="p-6">
          ...
          <div className={`border rounded-xl p-4 mb-5 ${cfg.bg}`}>
            ...
            <p className="text-sm text-white font-medium">{cita.fecha} — {cita.hora}</p>
            <p className="text-xs text-gray-400 mt-1">{cita.motivoConsulta ?? 'Sin motivo indicado'}</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {[
                  ...
                  <button key={value} type="button" onClick={() => setAccion(value)}
            ...
            {accion === 'cancelar' && (
              <div>
                <label className="block text-xs font-medium text-gray-400 mb-1">Motivo de cancelación</label>
            ...
            {(accion === 'confirmar' || accion === 'completar') && (
              <div>
                <label className="block text-xs font-medium text-gray-400 mb-1">Notas del staff</label>
            ...
            <div className="flex gap-3 pt-2">
              <button type="button" onClick={onClose}
                className="flex-1 py-2.5 bg-gray-800 hover:bg-gray-700 text-white text-sm rounded-lg">Cancelar</button>
              <button type="submit" disabled={loading || !accion}
                className="flex-1 py-2.5 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white text-sm font-medium rounded-lg">
    ...
    <div className="p-8">
      {modalSlots && (
        <ModalCrearSlots
          ...
          onCreado={() => { setModalSlots(false); buscar() }}
        />
      )}
      {citaSeleccionada && (
        <ModalGestionarCita
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={() => setModalSlots(true)} type="button"
      ...
      {pendientes.length > 0 && (
        <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-xl p-4 mb-6">
          <p className="text-sm font-medium text-yellow-400 mb-3">
            {pendientes.length} cita{pendientes.length > 1 ? 's' : ''} pendiente{pendientes.length > 1 ? 's' : ''} de confirmación
          </p>
              ...
              <div key={c.id} className="flex items-center justify-between bg-gray-900 rounded-lg px-4 py-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-400">{c.fecha} — {c.hora} · {c.motivoConsulta ?? 'Sin motivo'}</p>
                  </div>
                </div>
                <button onClick={() => setCitaSeleccionada(c)} type="button"
        ...
        <div className="flex items-end gap-4">
          ...
          <div className="flex gap-2 ml-auto">
            <button onClick={() => setVista('citas')}
              className={`px-3 py-2 text-xs rounded-lg transition-colors ${vista === 'citas' ? 'bg-blue-600 text-white' : 'bg-gray-800 text-gray-400 hover:text-white'}`}>
              Citas ({citas.length})
            </button>
            <button onClick={() => setVista('slots')}
              className={`px-3 py-2 text-xs rounded-lg transition-colors ${vista === 'slots' ? 'bg-blue-600 text-white' : 'bg-gray-800 text-gray-400 hover:text-white'}`}>
                ...
                <div key={c.id} className="bg-gray-900 border border-gray-800 rounded-xl p-5">
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-medium text-white">{c.nombrePaciente}</p>
                        <p className="text-xs text-gray-400">{c.numeroDocumento} · {c.emailPaciente ?? '—'}</p>
                        <div className="flex items-center gap-3 mt-1">
                          <span className="flex items-center gap-1 text-xs text-gray-300">
                            <Calendar size={11} />{c.fecha}
                          </span>
                          <span className="flex items-center gap-1 text-xs text-gray-300">
                ...
                {slots.map(s => (
                  <tr key={s.id} className="border-b border-gray-800/50 hover:bg-gray-800/30 transition-colors">
                    ...
                    <td className="px-5 py-3">
                      <span className={`px-2 py-1 rounded-md text-xs ${s.disponible ? 'bg-green-500/10 text-green-400' : 'bg-gray-500/10 text-gray-500'}`}>
                        {s.disponible ? 'Disponible' : 'Ocupado'}
                      </span>
                    </td>
                    <td className="px-5 py-3">
                      {s.disponible && (
                        <button onClick={() => eliminarSlot(s.id)} type="button"
