...
function ModalSolicitarCita({
  ...
  useEffect(() => {
    const cargarSlots = async () => {
      try {
        const res = await citasApi.getSlotsDisponibles(
          toLocalISOString(hoy, false),
          toLocalISOString(enUnMes, true)
        )
        setSlots(res)
      } catch (err: any) {
        setError(err?.response?.data?.mensaje ?? 'No se pudieron cargar los horarios disponibles')
      } finally {
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {slotsDelDia.map(s => (
                        <button
                          key={s.id}
                          type="button"
                          onClick={() => setSlotSeleccionado(s.id)}
          ...
          <div className="flex gap-3 pt-1">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 py-2.5 bg-gray-100 hover:bg-gray-200 text-gray-700 text-sm rounded-xl"
            >
              Cancelar
            </button>
            <button
                      ...
                      <div className="flex items-start justify-between gap-4">
                        <div>
                          <div className="flex items-center gap-2 mb-2">
                            <span
                              className={`px-2.5 py-0.5 rounded-full text-xs font-medium ${cfg.bg} ${cfg.color} border ${cfg.border}`}
                            >
                              {cfg.label}
                            </span>
                          </div>
                          ...
                          {c.motivoConsulta && (
                            <p className={`text-xs mt-2 ${cfg.color} opacity-80`}>
                              Motivo: {c.motivoConsulta}
                            </p>
                          )}

                          {c.notasStaff && (
                            <p className={`text-xs mt-1 ${cfg.color} opacity-80`}>
                              Indicaciones: {c.notasStaff}
                            </p>
                          )}
                        </div>
                        ...
                        <div className="flex items-center gap-3">
                          <div>
                            <div className="flex items-center gap-2">
                              ...
                              <span
                                className={`px-2 py-0.5 rounded-full text-xs ${cfg.bg} ${cfg.color} border ${cfg.border}`}
                              >
                                {cfg.label}
                              </span>
                            </div>
